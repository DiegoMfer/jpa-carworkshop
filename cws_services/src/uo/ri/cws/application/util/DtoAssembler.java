...
public class DtoAssembler {
	...
	public static InvoicingPaymentMeanDto toInvoicingPaymentMeanDto(
			PaymentMean pm) {
	...
	public static List<InvoicingPaymentMeanDto> toInvoicingPaymentMeanDtoList(
			List<PaymentMean> list) {
		return list.stream().map(pm -> toInvoicingPaymentMeanDto(pm))
	...
	public static List<WorkOrderDto> toWorkOrderDtoList(List<WorkOrder> list) {
		return list.stream().map(wo -> toDto(wo)).collect(Collectors.toList());
	}

	public static InvoicingWorkOrderDto toInvoicingWorkOrderDto(WorkOrder a) {
	...
	public static List<InvoicingWorkOrderDto> toInvoicingWorkOrderDtoList(
			List<WorkOrder> list) {
		return list.stream().map(wo -> toInvoicingWorkOrderDto(wo))
				.collect(Collectors.toList());
	}
	...
	public static VoucherSummaryDto toVoucherSummaryDto(
			List<Voucher> voucherList, Client client) {

		VoucherSummaryDto result = new VoucherSummaryDto();

		result.id = UUID.randomUUID().toString();
		result.version = client.getVersion();
		result.dni = client.getDni();
		result.name = client.getName();
		result.surname = client.getSurname();
		result.issued = voucherList.size();

		for (var i : voucherList) {
			result.totalAmount += i.getAccumulated() + i.getAvailable();
			result.consumed += i.getAccumulated();
			result.availableBalance += i.getAvailable();
		}
